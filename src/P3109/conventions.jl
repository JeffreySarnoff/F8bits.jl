export NaN8, Zero8, Inf8, 
       isinfnan, ispositive, isnonnegative, isnonpositive, isnegative
...
# Base.exponent_one(::Type{T}) where {T<:AbstractFloat} = exponent_mask(T) - sign_mask(T) >> 1
# Base.exponent_half(::Type{T}) where {T<:AbstractFloat} = (exponent_one(T) >> significand_bits(T) - 0x01) << significand_bits(T)

iszero(x::Float8{E}) where {E} = (value(x) === 0x00)
signbit(x::Float8{E}) where {E} = !iszero(value(x) & sign_mask(Float8{E}))
